    ...
    for directory in ['PartA', 'PartB', 'PartB-LowPass']:
        for file in os.listdir(directory):
            ...
            while index < len(df[0]):
                ...
                index += 6
            if os.path.isdir(directory + '/' + file[5:-4]):
                shutil.rmtree(directory + '/' + file[5:-4])
            os.mkdir(directory + '/' + file[5:-4])
            for i in range(len(time_lists)):
                with open(f'{directory}/{file[5:-4]}/meas{i + 1}.csv', 'w', newline='') as newfile:
